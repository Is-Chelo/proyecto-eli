const {promociones} = require('../models/index');
...
module.exports = {
	async create(body) {
		try {
			const response = await promociones.create(body);
	...
	async index(params = []) {
		try {
			let response = await promociones.findAll({});
	...
	async show(id) {
		try {
			const response = await promociones.findOne({
				where: {
					id: id,
				},
			});
			if (!response) return NotFoundResponse(`promociones con el id: ${id} no existe. `);
